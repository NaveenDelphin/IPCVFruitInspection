...
def minkowski_subtraction(img):
    ...
    cv2.imshow("Minkowski Edges",diff_contrast)
...
def segment_fruit(nir_image):
    ...
    cv2.imshow("OTSU Threshold",otsu_temp)
    ff1 = otsu_temp.copy()
    ...
    # cv2.imshow("holes",holes_temp)

    #Mask of the apples
    mask_temp = holes_temp | otsu_temp
    cv2.imshow("Binary Segmented Image",mask_temp)
...
def detect_defects(fruit_mask, mask_temp):
    ...
    # Using Minkowski Subtraction for edge extraction
    ...
    #Using Canny edge detection for robust edge detection
    img_blur = cv2.bilateralFilter(fruit_mask, 11, 35, 35)
    edge  = cv2.Canny(img_blur, threshold1=50, threshold2=120)
...
def contour_defects(open_temp, color_image):
    ...
    for contour in contours:
        ...
        cv2.circle(rounded_contours_img, center, radius + 17, (0, 255, 0), 2)
